<script lang="ts">
	...
	import { ArrowLeft, Bot, Save, Loader2, Trash2 } from 'lucide-svelte';
	...
	async function loadData() {
		...
		try {
			...
			// Redirect to agent page if not editable
			if (agent.source !== 'user') {
				goto(`/agents/${agentSlug}`);
				return;
			}
...
<svelte:head>
	<title>Edit {agent?.name || 'Agent'} | RAG Agent</title>
</svelte:head>
