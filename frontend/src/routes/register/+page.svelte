<script lang="ts">
	...
	async function handleSubmit(e: Event) {
		...
		if (!passwordsMatch) {
			toast.error(m.register_passwords_no_match());
			return;
		}

		if (password.length < 8) {
			toast.error(m.register_password_min());
			return;
		}
		...
		try {
			...
			toast.success(m.register_success?.() || 'Account created successfully!');
			goto('/chat');
		} catch (err) {
			if (err instanceof ApiException) {
				toast.error(err.message);
			} else {
				toast.error(m.common_error_unexpected());
			}
		} finally {
			isLoading = false;
		}
				...
				<div class="space-y-2">
					...
					<div class="relative">
						<Input
							...
							required
							disabled={isLoading}
							class="pr-10 {!passwordsMatch ? 'border-destructive' : ''}"
						/>
						<button
							type="button"
							class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground"
