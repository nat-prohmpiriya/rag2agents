<script lang="ts">
	...
	import { createCheckout, createPortalSession, type BillingPlan } from '$lib/api/billing';
	import type { UserUsage } from '$lib/api/profile';
	import {
		CreditCard,
		Zap,
		FileText,
	...
	let currentPlan = $derived(plans.find(p => p.plan_type === userTier) || plans.find(p => p.plan_type === 'free') || null);
